...
2. **Lớp Xử lý Băm & Biến đổi Bit (Hashing & Bit-manipulation Layer):**
   - Hàm `hash_function`: Tạo các hàm băm độc lập thông qua chuỗi kết hợp `seed + data` qua MD5, đưa về số nguyên 32-bit.
   - Hàm `count_trailing_zeros`: Sử dụng phép toán thao tác bit (`bit-shift` và toán tử `&`) để đếm số bit 0 ở cuối biểu diễn nhị phân với tốc độ tối ưu.
3. **Lớp Thuật toán Ước lượng (Estimation Core Algorithms):**
   ...
   - `FlajoletMartinAdvanced`: Hiện thực thuật toán FM cải tiến với $m = 128$ hàm băm, chia thành 16 nhóm. Áp dụng trung bình số học (Mean) trong từng nhóm và trung vị (Median) giữa các nhóm để loại trừ giá trị ngoại lai.
...
2. Địa chỉ IP được tách ra và đồng thời đưa vào 3 bộ xử lý:
   ...
   - `fm_advanced.update(ip)` (FM nâng cao: Băm 128 lần với 128 seed khác nhau -> Đếm bit 0 cuối -> Cập nhật mảng `max_zeros[128]`).
...
| `count_trailing_zeros(n)` | `n: int` (32-bit) | `int` | Đếm số lượng bit 0 liên tiếp ở cuối biểu diễn nhị phân |
| `hash_function(data_str, seed)` | `data_str: str, seed: int` | `int` (32-bit) | Tạo mã băm nguyên 32-bit độc lập qua MD5 và seed |
...
| `FlajoletMartinAdvanced.update(item)` | `item: str` | `None` | Cập nhật mảng 128 giá trị bit 0 đuôi lớn nhất |
| `FlajoletMartinAdvanced.estimate()` | Không có | `float` | Ước lượng số phần tử bằng kỹ thuật Median of Means |
...
graph TD
    ...
    subgraph "Xử lý Xác suất (Flajolet-Martin)"
        ...
        C -->|"IP"| F["FlajoletMartinAdvanced (128 Hash)"]
        F --> F1["128 Hàm băm MD5 độc lập"]
        F1 --> F2["Mảng max_zeros kích thước 128"]
        F2 --> F3["Chia 16 nhóm: Tính Trung bình (Mean)"]
        F3 --> F4["Tính Trung vị các nhóm (Median)"]
    end
